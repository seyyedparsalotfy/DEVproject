...
const ExampleComponent = () => {
    ...
    misc_expenses: [
      {
        misc_id: 10740,
        description: "تست فنی",
        datetime: "2022-04-20T16:57:30.703642+04:30",
          amount: 120000
      }
  ],
  concurrency_costs: [
      {
        concurrency_id: 28705,
        datetime : "2022-04-20T16:58:54.148113+04:30",
          amount: 7140000,
  ...
  const combinedData = [...data.trip_financials, ...data.payments, ...data.misc_expenses,...data.concurrency_costs];
  ...
  const sortedDates = Object.keys(groupedData).sort(
    (a, b,c,d) =>new Date(d).getTime()- new Date(c).getTime() - new Date(b).getTime() - new Date(a).getTime(),
  );
  //
  ...
  const filteredData = (filterType) => {
    if (filterType === "payment") {
      ...
      return combinedData.filter((item) => item.trip_id !== undefined);
     } else if (filterType === "misc_expenses") {
        return combinedData.filter((item) => item.misc_id !== undefined);
      } else if (filterType === "concurrency_costs") {
        return combinedData.filter((item) => item.concurrency_id !== undefined);
    } else {
      return combinedData;
    }
  };
  ...
  return (
    <div>
      <div>
        ...
        <button onClick={() => setFilterType("misc_expenses")}>Filter by misc_expenses </button>
        <button onClick={() => setFilterType("concurrency_costs")}>Filter by concurrency_costs </button>
      </div>
